...
def create_simplified_matviews_appsimplify_table_app(
    table: str = Argument(..., help='The table to simplify.'),
    ...
    with Postgres(host=host, database=database, user=aoetl_db_config.database.user) as db_src:
        db_src.create_simplified_matviews(table=table, source_schema=source_schema, 
                              simplify_tolerances=simplify_tolerances,
                              dest_schema=dest_schema, dryrun=dryrun)


if __name__ == '__main__':
    typer.run(create_simplified_matviews_appsimplify_table_app)
